...
UNION
SELECT P.PERMALINK, '/page/program/'||P.PERMALINK AS LINK_DETAIL, P.TITLE, P.IMAGE_URL AS IMAGE_URL, 'Program Post' AS TYPE, 2 AS ORDER_NO FROM PROGRAM_POST P WHERE P.STATUS = 2
UNION
...
ELSE PI.IMAGE_URL END) AS IMAGE_URL,
 c.title AS TYPE, 6 AS ORDER_NO FROM PROGRAM_ITEM PI 
...
(CASE WHEN PI.IMAGE_THUMB_URL IS NOT NULL THEN PI.IMAGE_THUMB_URL
ELSE PI.IMAGE_URL END) AS IMAGE_URL,
c.title AS TYPE, 7 AS ORDER_NO FROM PROGRAM_ITEM PI 
INNER JOIN PROGRAM_ITEM_CATEGORY PIC ON PIC.FK_PROGRAM_ITEM = PI.PK_PROGRAM_ITEM
INNER JOIN CATEGORY C ON C.PK_CATEGORY = PIC.FK_CATEGORY
...
R.IMAGE_THUMB_URL AS IMAGE_URL, c.title AS TYPE, 8 AS ORDER_NO FROM RESEARCH R 
...
ALTER TABLE LINK_URL ADD COLUMN IS_PARENT BOOLEAN DEFAULT FALSE;
ALTER TABLE LINK_URL ADD COLUMN PERMALINK TEXT;